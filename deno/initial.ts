...
export const getInitial = async (ctx: any) => {
  ...
  const allSongs = await Song.all()

  const allSongsCollection = {
    id: 'allsongs',
    title: 'All songs',
    songs: allSongs
  }
  ...
  ctx.response.body = JSON.stringify([allSongsCollection, artistCollection3, albumCollection2, artistCollection4, albumCollection1, artistCollection3, artistCollection1, artistCollection3, artistCollection2, albumCollection2, artistCollection4, artistCollection1, artistCollection3])
}